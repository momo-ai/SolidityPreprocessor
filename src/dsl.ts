...
function process_ContractDefinition(node:ContractDefinition): void {
    let baseIds:number[] = [];
    const newContract:ContractDefinition = new ContractDefinition(id++, srcStr, node.name, node.scope, node.kind, node.abstract, node.fullyImplemented, baseIds, node.usedErrors, node.documentation, [], node.nameLocation);
    addNode(node, newContract);

    for(const baseId of node.linearizedBaseContracts.reverse()) {
        baseIds.unshift(getNewId(node, baseId));
    }

    const oldContract:ContractDefinition = curContract;
    curContract = node;
    visitChildren(node, newContract);
    curContract = oldContract;
}

function process_ImportDirective(node:ImportDirective): void {
    getNewId(node, node.sourceUnit);
    //skip
}
...
export function preprocess(units: SourceUnit[]):SourceUnit {
